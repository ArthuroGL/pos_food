...
<div class="card shadow-sm mb-4" id="comanda-{{ $comanda->id }}">
    <div class="card-header bg-primary text-white d-flex justify-content-between">
        <div>
            <strong>Mesa:</strong> {{ $comanda->mesa->nombre }}
            <span class="badge bg-light text-dark ms-2">
                {{ $comanda->productos->where('pivot.estado', 'pendiente')->count() }} pendientes
            </span>
        </div>
        <small>Última actualización: {{ $comanda->updated_at->diffForHumans() }}</small>
    </div>
    ...
    <div class="card-body">
        <div class="row">
            ...
            <div class="col-md-6 border-end">
                ...
                <div class="alert alert-warning producto-item" id="producto-{{ $comanda->id }}-{{ $producto->id }}">
                    <div class="row align-items-start">
                        ...
                        <div class="row align-items-start d-flex gap-3">
                            <strong>{{ $producto->nombre }}</strong> x {{ $producto->pivot->cantidad }}<br>
                            @if($producto->pivot->comentarios)
                            <small class="text-muted "><em>{{ $producto->pivot->comentarios }}</em></small>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="alert alert-success mb-2">
                    <div class="d-flex justify-content-between">
                        <div>
                            <strong>{{ $producto->nombre }}</strong> x {{ $producto->pivot->cantidad }}<br>
                            @if($producto->pivot->comentarios)
                            <small><em>{{ $producto->pivot->comentarios }}</em></small>
                            @endif
                        </div>
