...
@section('view_title', '🪪 Expediente de Personal')
@section('view_subtitle', 'Visualización de datos generales del empleado y gestión de identidad visual')
...
<div class="h-full w-full p-6 overflow-y-auto sheet-layout bg-slate-100 text-slate-800 scroll-custom flex flex-col">
    ...
    <div class="w-full max-w-4xl mx-auto flex flex-col flex-1">
        ...
        {{-- GRID PRINCIPAL: Avatar e Información --}}
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-start w-full mb-6">

            {{-- TARJETA 1 (Izquierda): Gestión de Imagen de Perfil --}}
...
{{-- Formulario apuntando a la ruta POST real --}}
<form action="{{ route('users.updateAvatar', $user->id) }}" method="POST" enctype="multipart/form-data" class="w-full m-0">
    @csrf
    ...
    {{-- Botón de confirmación que el JS activa al detectar cambio --}}
    <button type="submit"
            id="btn-save-avatar"
        ...
        <i class="fas fa-check text-xs"></i> Confirmar Cambio
            ...
            {{-- TARJETA 2 (Derecha): Desglose de Información General --}}
            <div class="bg-white border border-slate-200 rounded-2xl shadow-3xs overflow-hidden md:col-span-2 h-full flex flex-col">
                <div class="p-6 flex-1 space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <div class="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-mono font-bold text-slate-700 truncate" title="{{ $user->email }}">
                                {{ $user->email }}
                            </div>
                        </div>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <div class="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3">
                                ...
                                <span class="text-xs font-black uppercase tracking-wide text-slate-800">
                                    {{ $user->getRoleNames()->implode(', ') ?: 'Sin Rol Comercial' }}
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="px-6 py-3.5 bg-slate-50 border-t border-slate-200 flex items-center justify-between text-[11px] font-medium text-slate-400">
                    ...
                    <span>ID de Control: #{{ str_pad($user->id, 5, '0', STR_PAD_LEFT) }}</span>
                </div>
            </div>
