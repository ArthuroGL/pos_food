...
<div class="w-full p-4 min-h-screen bg-slate-50 text-slate-800 selection:bg-orange-500 selection:text-white">
   ...
   <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    ...
    {{-- GRID PRINCIPAL DE CREACIÓN --}}
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">

        {{-- Panel de la Comanda (Derecha en pantallas grandes) --}}
        <div class="w-full lg:col-span-4 order-1 lg:order-2 lg:sticky lg:top-4">
            <form id="mainComandaForm" method="POST" action="{{ route('comandas.store') }}" class="m-0">
                @csrf
                    ...
                    <div class="p-4 bg-white border-b border-slate-100">
                        ...
                        <div class="relative">
                            ...
                            <select name="mesa_id" id="mesa_id" class="w-full pl-9 pr-10 py-3 bg-slate-50 border border-slate-200 focus:bg-white focus:border-orange-400 focus:ring-4 focus:ring-orange-100/60 text-slate-900 font-black text-sm rounded-xl outline-none transition-all cursor-pointer appearance-none" required>
                                ...
                                @php $ocupada = $mesa->comandas->isNotEmpty(); @endphp
                                <option value="{{ $mesa->id }}"
                                    @if($ocupada) disabled class="text-rose-500 font-medium bg-rose-50/50" @endif
                                    @if(old('mesa_id')==$mesa->id) selected @endif>
                                    📍 {{ $mesa->nombre }} @if($ocupada) [Ocupada] @endif
                                </option>
                                @endforeach
                            </select>
        ...
        {{-- Panel de Productos (Izquierda en pantallas grandes) --}}
        <div class="w-full lg:col-span-8 order-2 lg:order-1">

            {{-- BUSCADOR AVANZADO Y COMERCIAL --}}
            <div class="bg-white p-3 rounded-2xl border border-slate-200 shadow-xs mb-6 relative">
                <form method="GET" action="{{ route('comandas.create') }}" id="searchForm" class="m-0">
                    <div class="relative flex items-center w-full">
                        ...
                        <button type="button" id="clearSearchBtn"
                            class="absolute right-26 text-slate-400 hover:text-slate-600 p-1 rounded-full hover:bg-slate-100 transition-all cursor-pointer {{ request('search') ? '' : 'hidden' }}">
                ...
                <div id="suggestionsDropdown" class="absolute left-0 right-0 mt-2 bg-white border border-slate-200 rounded-xl shadow-lg z-50 overflow-hidden hidden max-h-60 overflow-y-auto">
                    {{-- Las sugerencias o el estado vacío se renderizan dinámicamente aquí --}}
                </div>
            </div>
            ...
            {{-- Grid interna de Tarjetas de Productos --}}
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                @if ($productos->count() > 0)
                @foreach ($productos as $producto)
                <div class="product-card border border-slate-200 bg-white rounded-2xl overflow-hidden shadow-xs hover:shadow-md transition-all duration-200 flex flex-col justify-between" id="card_container_{{ $producto->id }}">

                    <div class="p-4 flex gap-3 items-start relative">
                        <input type="checkbox" name="productos[{{ $producto->id }}][selected]" value="1"
                        ...
                        <div class="min-w-0 flex-1">
                            <h3 class="text-sm font-black text-slate-900 tracking-tight leading-snug truncate m-0">{{ $producto->nombre }}</h3>
                            <p class="text-xs font-medium text-slate-400 mt-1 line-clamp-2 leading-normal h-8 m-0">{{ $producto->descripcion }}</p>
                            <span class="inline-block text-sm font-mono font-black text-orange-600 mt-1.5">${{ number_format($producto->precio, 2) }}</span>
                        </div>
                    </div>
                    ...
                    <div class="p-3 bg-slate-50/80 border-t border-slate-100 space-y-2.5">
                        <div class="flex items-center justify-between gap-2">
                            ...
                            <div class="flex items-center bg-white border border-slate-200 rounded-lg p-0.5 shadow-xs">
                                <button type="button" onclick="stepDownQty('{{ $producto->id }}')" class="w-7 h-7 flex items-center justify-center rounded bg-slate-50 hover:bg-slate-100 border-0 text-slate-600 text-xs font-bold transition-all cursor-pointer select-none">-</button>
                                <input type="number" name="productos[{{ $producto->id }}][cantidad]" id="qty_{{ $producto->id }}"
                                ...
                                <button type="button" onclick="stepUpQty('{{ $producto->id }}')" class="w-7 h-7 flex items-center justify-center rounded bg-slate-50 hover:bg-slate-100 border-0 text-slate-600 text-xs font-bold transition-all cursor-pointer select-none">+</button>
                            </div>
                        </div>

                        <div class="relative flex items-center">
                            <i class="fas fa-pen absolute left-3 text-slate-300 text-[9px]"></i>
                            <input type="text" name="productos[{{ $producto->id }}][comentarios]" id="comment_{{ $producto->id }}"
                        ...
                        <button type="button" id="btn_add_{{ $producto->id }}" onclick="toggleProductSelection('{{ $producto->id }}')"
...
<script>
    ...
    if (searchInput) {
        ...
        clearSearchBtn.addEventListener('click', () => {
            ...
            window.location.href = "{{ route('comandas.create') }}";
        });
