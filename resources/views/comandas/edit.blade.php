...
<div class="w-full p-4 min-h-screen bg-slate-50 text-slate-800 selection:bg-orange-500 selection:text-white">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-xs font-medium text-slate-500 m-0 mt-0.5">
                Modificando la orden activa de la <span class="font-black text-slate-700">📍 {{ $comanda->mesa->nombre }}</span>
            </p>
        </div>

        {{-- BADGE DE ESTADO ACTUAL --}}
        {{-- Botón Universal Reutilizable --}}
    <div class="shrink-0">
        <a href="{{ route('comandas.index') }}" class="btn-back-nav no-underline">
    ...
    <form id="editComandaForm" method="POST" action="{{ route('comandas.update', $comanda) }}" class="m-0">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            {{-- PANEL IZQUIERDO: BUSCADOR, CATEGORÍAS Y CATÁLOGO DE PRODUCTOS --}}
            <div class="w-full lg:col-span-8 order-2 lg:order-1">
                {{-- REPLICACIÓN EXACTA DEL BUSCADOR AVANZADO Y COMERCIAL --}}
                <div class="bg-white p-3 rounded-2xl border border-slate-200 shadow-xs mb-6 relative">
                    <div class="relative flex items-center w-full">
                        ...
                        {{-- Botón interactivo para limpiar la búsqueda --}}
                    ...
                    {{-- Dropdown Moderno de Sugerencias en Tiempo Real --}}
                ...
                {{-- NAVEGADOR HORIZONTAL DE CATEGORÍAS REPLICADO --}}
                ...
                <div class="mb-6">
                    ...
                    <div class="flex items-center gap-2 overflow-x-auto pb-2 scroll-custom" id="categoriesContainer">
                        ...
                        <button type="button" onclick="filterCategory('{{ $cat->id }}')" id="cat_btn_{{ $cat->id }}"
                ...
                {{-- Contenedor del Catálogo con filtrado dinámico en tiempo real --}}
                <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4" id="catalogGrid">
                    ...
                    <div class="product-card border border-slate-200 bg-white rounded-2xl overflow-hidden shadow-xs hover:shadow-md transition-all duration-200 flex flex-col justify-between"
                         id="card_container_{{ $producto->id }}"
                         data-nombre="{{ strtolower($producto->nombre) }}"
                         data-descripcion="{{ strtolower($producto->descripcion) }}"
                         data-categoria="{{ $producto->categoria_id ?? 'all' }}">
                        ...
                        <div class="p-4 flex gap-3 items-start relative">
                            <input type="checkbox"
                                   ...
                                   onclick="syncExtraCardState('{{ $producto->id }}')">
                            ...
                            <div class="min-w-0 flex-1">
                                <label for="check_{{ $producto->id }}" class="text-sm font-black text-slate-900 tracking-tight leading-snug truncate block m-0 cursor-pointer hover:text-orange-600 transition-colors">
                                    {{ $producto->nombre }}
                                </label>
                                <p class="text-[11px] font-medium text-slate-400 mt-0.5 line-clamp-2 leading-tight h-7 m-0">{{ $producto->descripcion }}</p>
                                <span class="inline-block text-xs font-mono font-black text-orange-600 mt-1">${{ number_format($producto->precio, 2) }}</span>
                            </div>
                        </div>
                        ...
                        <div class="p-3 bg-slate-50/80 border-t border-slate-100 space-y-2.5">
                            <div class="flex items-center justify-between gap-2">
                                ...
                                <div class="flex items-center bg-white border border-slate-200 rounded-lg p-0.5 shadow-xs">
                                    <button type="button" onclick="stepDownExtra('{{ $producto->id }}')" class="w-7 h-7 flex items-center justify-center rounded bg-slate-50 hover:bg-slate-100 border-0 text-slate-600 text-xs font-bold transition-all cursor-pointer select-none">-</button>
                                    <input type="number"
                                    ...
                                    <button type="button" onclick="stepUpExtra('{{ $producto->id }}')" class="w-7 h-7 flex items-center justify-center rounded bg-slate-50 hover:bg-slate-100 border-0 text-slate-600 text-xs font-bold transition-all cursor-pointer select-none">+</button>
                                </div>
                            </div>
                            ...
                            <button type="button" id="btn_add_{{ $producto->id }}" onclick="toggleExtraSelection('{{ $producto->id }}')"
                ...
                {{-- Estado de Búsqueda Vacío Integrado --}}
            ...
            {{-- PANEL DERECHO: DETALLE FIJO DE LA ORDEN ACTIVA --}}
                ...
                <div class="border border-slate-200 shadow-sm rounded-2xl overflow-hidden bg-white flex flex-col">
                    ...
                    <div class="p-4 bg-slate-900 border-b border-slate-800 flex justify-between items-center">
                        ...
                        <span class="px-2 py-0.5 text-[10px] font-mono font-black text-slate-300 bg-slate-800 rounded-md">
                            Mesa: {{ $comanda->mesa->nombre }}
                        </span>
                    </div>

                    {{-- Historial de lo que ya está enviado a cocina --}}
                    ...
                    {{-- Lista dinámica en tiempo real de los EXTRAS que se están sumando en este instante --}}
                    ...
                    <div class="p-4 bg-slate-50 border-t border-slate-100 space-y-2">
                        ...
                        <a href="{{ route('comandas.index') }}" onclick="limpiarAlmacenEdicion()" class="w-full inline-flex items-center justify-center gap-2 py-2.5 bg-white border border-slate-200 hover:bg-slate-100 active:scale-[0.98] text-slate-700 text-xs font-black tracking-wider uppercase rounded-xl transition-all text-decoration-none">
...
<script>
    ...
    const storageKey = 'extras_comanda_{{ $comanda->id }}';
    const categoryStorageKey = 'categoria_activa_{{ $comanda->id }}';
    ...
    if (searchInput) {
        searchInput.addEventListener('input', function() {
            ...
            debounceTimer = setTimeout(() => {
                fetch(`{{ route('productos.sugerencias') }}?search=${encodeURIComponent(query)}`)
